...
class Eval():
    def __init__(self):
        ...
        self.info_dir = os.path.join('test_set', 'test_info.json')
        self.seg_labels_dir = os.path.join('test_set', 'seg_result_100_new.json')
        self.lianguan_labels_dir = os.path.join('test_set', 'lianguan_result_100_new.json')

        self.visual_features_dir = os.path.join('test_set', 'swin_features.pkl')
        self.text_features_dir = os.path.join('test_set', 'bert_features.pkl')
        self.audio_features_dir = os.path.join('test_set','vggish_features.pkl')
    ...
    def score_total(self, md5, result, dur_vec, given_time):
        return self.score_adsprop(md5, result)*self.score_cor(md5, result)*self.score_time(md5, result, dur_vec, given_time)
        ...
        for inp_vec, dur_vec, n_fragment, md5, _ in self.ld:
            for i in range(len(md5)):
                if sum(dur_vec[i]) < given_time[0]:
                    continue
                cnt += 1
                tmp_time_score = self.score_time(md5[i], results[i], dur_vec[i], given_time)
